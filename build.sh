...
then
  ...
  then
    ...
    git submodule add git@github.com:castlestechusa/luna luna
    tar czf luna.tgz luna
  fi
fi
...
if [ "x$NOS3" = "x" ]
then
  aws s3 sync $artifacts_dir s3://castlestw/luna/tip/$target
fi

cd luna
aws s3 cp s3://castlestw/luna/tip/$target/index.json ./index.json
...
aws s3 cp ./index.json s3://castlestw/luna/tip/$target/index.json
